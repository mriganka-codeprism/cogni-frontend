...
const CandidateEvaluation = () => {
  ...
  const [college, setCollege] = useState(
    sanitizeId(storedFiltersRef.current?.college ?? "")
  );
  const normalizeSelectedColleges = (raw: any): string[] => {
    if (!raw) return [];
    try {
  ...
  const [selectedColleges, setSelectedColleges] = useState<string[]>(
    normalizeSelectedColleges(storedFiltersRef.current?.selectedColleges).map(sanitizeId).filter(Boolean)
  );

  // Persist a sanitized version of stored filters back to sessionStorage to avoid
  // future loads with string 'undefined' or 'null' values.
  useEffect(() => {
    try {
      const payload: CandidateEvaluationFilters = {
        search: storedFiltersRef.current?.search ?? "",
        college: sanitizeId(college) || undefined,
        selectedColleges: (selectedColleges && selectedColleges.length > 0) ? selectedColleges : undefined,
        selectedCandidateStatus: storedFiltersRef.current?.selectedCandidateStatus ?? undefined,
        page: storedFiltersRef.current?.page ?? undefined,
        sortBy: storedFiltersRef.current?.sortBy ?? undefined,
        sortOrder: storedFiltersRef.current?.sortOrder ?? undefined,
        startDate: storedFiltersRef.current?.startDate ?? undefined,
        endDate: storedFiltersRef.current?.endDate ?? undefined,
      };
  ...
  // Infinite scroll state for college dropdown
  const [collegePageCache, setCollegePageCache] = useState<
    Record<number, { id: string; name: string }[]>
  >({});
  const [collegeHasMore, setCollegeHasMore] = useState(true);
  const [collegeLoadingPage, setCollegeLoadingPage] = useState(false);
  const collegeCurrentPageRef = useRef(1);
  const collegePageCacheRef = useRef<Record<number, { id: string; name: string }[]>>({});
  const collegeDropdownScrollRef = useRef<HTMLDivElement>(null);
  ...
  useEffect(() => {
    ...
    const needsNormalization = selectedColleges.some(
      (v) => typeof v !== "string"
    );
    if (needsNormalization) {
      const normalized = selectedColleges.map((v) => {
        try {
  ...
  // Calculate button disabled state based on stats, selected colleges, and evaluation progress
  useEffect(() => {
    // Extract "NOT EVALUATED" and "DEFAULTED" counts from existing stats (already college-filtered)
  ...
  }, [stats, selectedColleges, progressPercentage]);
  ...
  const fetchEvaluationData = useCallback(async () => {
    try {
      ...
      const response = await getEvaluation(
        ...
        // Only include start/end dates when they are valid Dayjs objects
  ...
  // Fetch colleges with pagination for infinite scroll
  const fetchCollegesPaginated = useCallback(async (pageNum: number) => {
    if (collegeLoadingPage || !collegeHasMore) return;

    // Prevent re-fetching same page
    if (collegePageCacheRef.current[pageNum]) return;

    setCollegeLoadingPage(true);
    try {
      const res = await getColleges(pageNum, 50);
      if (res && res.data && Array.isArray(res.data)) {
        // Filter out colleges with empty streamIds
        const filteredColleges = res.data.filter(
          (col: { streamIds?: string[] }) => col.streamIds && col.streamIds.length > 0
        );

        // Update cache ref and state
        collegePageCacheRef.current[pageNum] = filteredColleges;
        setCollegePageCache({ ...collegePageCacheRef.current });

        // Determine if more pages exist (assume 50 items per page)
        const hasMore = filteredColleges.length === 50;
        setCollegeHasMore(hasMore);

        // Increment page ONLY after successful fetch
        collegeCurrentPageRef.current = pageNum + 1;
      }
    } catch (error) {
      console.error("Failed to fetch colleges:", error);
    } finally {
      setCollegeLoadingPage(false);
    }
  }, [collegeLoadingPage, collegeHasMore]);

  // Rebuild college list whenever cache changes
  useEffect(() => {
    const pages = Object.keys(collegePageCache)
      .map(Number)
      .sort((a, b) => a - b);

    const allColleges = pages.flatMap((page) => collegePageCache[page]);
    setCollegeList(allColleges);
  }, [collegePageCache]);

  // Handle infinite scroll in dropdown
  const handleCollegeListScroll = useCallback(
    (event: React.UIEvent<HTMLDivElement>) => {
      ...
      if (scrollHeight - scrollPosition < 100) {
        fetchCollegesPaginated(collegeCurrentPageRef.current);
      }
    },
    [collegeLoadingPage, collegeHasMore, fetchCollegesPaginated]
  );

  // Reset pagination when search changes
  useEffect(() => {
    collegeCurrentPageRef.current = 1;
    collegePageCacheRef.current = {};
    setCollegePageCache({});
    setCollegeHasMore(true);
    setCollegeLoadingPage(false);
  }, [debouncedCollegeSearch]);

  // Load initial colleges on mount
  useEffect(() => {
    if (Object.keys(collegePageCache).length === 0) {
      fetchCollegesPaginated(1);
    }
  }, [fetchCollegesPaginated, collegePageCache]);
  ...
  // Recalculate "Start Evaluation" button enabled state based on stats and selected colleges
  useEffect(() => {
    const hasSelectedColleges = selectedColleges.length > 0;
  ...
  const handleGenerateEvaluationReport = async () => {
    try {
      ...
      // Call the API with college ID only
      // The backend will handle filtering unevaluated candidates for the entire college (not paginated subset)
      await generateEvaluationReport(college);
  ...
  useEffect(() => {
    ...
    const handleBatchEvaluation = async () => {
      try {
        const response = await getBatchEvaluationProgress(college);
  ...
  }, [college, fetchBatchEvaluationProgress]);
  ...
  const handleDownloadCsv = async () => {
    ...
    try {
      ...
      // Derive a human-friendly filename using selected college name(s)
      const selectedCollegeName = (() => {
        if (!selectedColleges || selectedColleges.length === 0) return "all";
        if (selectedColleges.length === 1) {
          const col = collegeList.find((c) => c.id === selectedColleges[0]);
          return col?.name || "all";
        }
        return `${selectedColleges.length}_colleges`;
  ...
  const orderedCollegeList = useMemo(() => {
    ...
    const filteredCollegeList = normalizedCollegeSearch
      ? collegeList.filter((col) =>
        col.name.toLowerCase().includes(normalizedCollegeSearch)
      )
      : collegeList;
    const selectedCollegeEntries = selectedColleges
      .map(
        (id) =>
          collegeLookupRef.current[id] ||
  ...
  }, [collegeSearch, collegeList, selectedColleges]);
          ...
          <Box>
            ...
            <Select
              ...
              onChange={(e) => {
                ...
                setSelectedColleges(cleaned);
                const collegeVal = cleaned.length === 1 ? cleaned[0] : "";
              ...
              renderValue={(selected) => {
                ...
                if (!ids || ids.length === 0) return "All Colleges";
                if (ids.length === 1) {
                  const id = ids[0];
                  ...
                  if (!col) {
                    col =
                      collegeList.find((c) => String(c.id) === String(id)) ||
                      collegeList.find((c) => String(c.id) === String(Number(id))) ||
                      undefined;
                  }
                  return col?.name || "Unknown College";
                }
                return `${ids.length} colleges selected`;
              }}
            >
              <ListSubheader
                component="div"
              ...
              >
                ...
                <Box sx={{ display: "flex", gap: "0.5vh" }}>
                  <Button
                    ...
                    onClick={(e) => {
                      e.stopPropagation();
                      const allCollegeIds = collegeList.map((c) => c.id);
                      setSelectedColleges(allCollegeIds);
                      setCollege(allCollegeIds.join(","));
                      setIsEvaluationButtonEnabled(false);
                      setProgressPercentage(null);
                    }}
                    sx={{
                  ...
                  <Button
                    ...
                    onClick={(e) => {
                      ...
                      setSelectedColleges([]);
                      setCollege("");
                      setCollegeSearch(""); // Clear search input as well
                      setIsEvaluationButtonEnabled(false);
                      setProgressPercentage(null);
                    }}
                    sx={{
              ...
              >
                {collegeList
                  .filter((col) =>
                    !debouncedCollegeSearch ||
                    col.name.toLowerCase().includes(debouncedCollegeSearch)
                  )
                  .map((col) => {
                    const isSelected = selectedColleges.indexOf(col.id) > -1;
                    return (
                      <MenuItem
                        key={col.id}
                        value={col.id}
                        sx={{ fontSize: "1.2vh", py: "0.6vh" }}
                        onClick={(e) => {
                          e.stopPropagation();
                          // Manual toggle logic: don't rely on Select onChange
                          let updatedColleges: string[];
                          if (isSelected) {
                            // Remove from selection
                            updatedColleges = selectedColleges.filter(
                              (id) => id !== col.id
                            );
                          } else {
                            // Add to selection
                            updatedColleges = [...selectedColleges, col.id];
                          }
                          // Update both selectedColleges and college state
                          setSelectedColleges(updatedColleges);
                          setCollege(updatedColleges.join(","));
                          setIsEvaluationButtonEnabled(false);
                          setProgressPercentage(null);
                        }}
                      >
                        ...
                        />
                        <ListItemText
                          primaryTypographyProps={{ fontSize: "1.2vh" }}
                          primary={col.name}
                        />
                      </MenuItem>
                    );
                ...
                {collegeLoadingPage && (
                  <Box sx={candidateEvaluationStyles.dropdownLoadingBox}>
                    <CircularProgress
                      size={24}
                      sx={candidateEvaluationStyles.dropdownLoadingSpinner}
                    />
                  </Box>
                )}
                {/*
                {!collegeHasMore && collegeList.length > 0 && (
                  <Box sx={candidateEvaluationStyles.endOfListMessage}>
                    All colleges loaded
                  </Box>
                )}
              */}
      ...
      <Box sx={candidateEvaluationStyles.candidateDetailsBox}>
        ...
        <Box sx={candidateEvaluationStyles.candidateDetailsRightBox}>
          <Tooltip
            ...
            title={
              !selectedColleges.length
                ? "Please select a college to start evaluation"
                : (() => {
                  ...
                  return notEvalCount === 0 && defaultCount === 0
                    ? "All candidates in the selected college(s) are already evaluated or defaulted"
                    : "";
                })()
            }
            ...
            <TableRow>
              ...
              <TableCell
                sx={{
                  ...
                  width: "10%",
                  "&:hover .sortIcon": {
                    opacity: sortBy === "collegeName" ? 1 : 0.5,
                  },
                }}
              >
                <Box
                ...
                evaluationData.data.map((row, idx) => (
                  ...
                  >
                    ...
                    <TableCell sx={candidateEvaluationStyles.tableDataCell}>
                      <Tooltip title={row.collegeName} arrow placement="top">
